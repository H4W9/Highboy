...
// Pancake ST7796S is a full 320x480 panel — no truncation gap needed.
#define ROTATION_GAP_OFFSET 0
...
void lcd_set_rotation(uint8_t rotation) {
  ...
  switch (rotation) {
    case 1:
      // Pancake ST7796S: mirror X so text reads correctly (panel column order
      // is reversed relative to the framebuffer).
...
void st7789_init(void) {
  // DC (GPIO3) and RST (GPIO2) are ESP32-C5 strapping pins — release their
  // boot-time strapping function before driving them.
  gpio_reset_pin(GPIO_ST7789_DC_PIN);
  gpio_reset_pin(GPIO_ST7789_RST_PIN);
  ...
  // Pancake shares SPI2_HOST between the ST7796S panel and the SD card.
  ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi(SPI2_HOST, &io_config, &io_handle));

  esp_lcd_panel_dev_config_t panel_config = {
      .reset_gpio_num = GPIO_ST7789_RST_PIN,
      // This ST7796S panel is wired BGR — RGB order shows red/blue swapped
      // (accents appear magenta/pink).
      .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_BGR,
      .bits_per_pixel = 16,
  };
  // ST7796S responds to the same standard MIPI-DCS command set as the
  // built-in esp_lcd ST7789 vendor init (SWRESET/SLPOUT/COLMOD/MADCTL/
  // CASET/RASET/RAMWR/DISPON), so we reuse it. esp_lcd_st7796 has no
  // esp32c5 build, hence this approach for the Pancake single-chip target.
  ESP_ERROR_CHECK(esp_lcd_new_panel_st7789(io_handle, &panel_config, &panel_handle));
  ESP_ERROR_CHECK(esp_lcd_panel_reset(panel_handle));
  ESP_ERROR_CHECK(esp_lcd_panel_init(panel_handle));
  // This ST7796S panel needs color inversion on (same as Highboy's IPS ST7789).
